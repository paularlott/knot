...
func (db *RedisDbDriver) SaveAgentState(state *model.AgentState) error {
  ...
  return db.connection.Set(context.Background(), fmt.Sprintf("AgentState:%s", state.Id), data, model.AGENT_STATE_TIMEOUT).Err()
}

func (db *RedisDbDriver) DeleteAgentState(state *model.AgentState) error {
  return db.connection.Del(context.Background(), fmt.Sprintf("AgentState:%s", state.Id)).Err()
}

func (db *RedisDbDriver) GetAgentState(id string) (*model.AgentState, error) {
  var state = &model.AgentState{}

  v, err := db.connection.Get(context.Background(), fmt.Sprintf("AgentState:%s", id)).Result()
  if err != nil {
