...
func (db *RedisDbDriver) SaveTemplate(template *model.Template) error {
  ...
  return db.connection.Set(context.Background(), fmt.Sprintf("Templates:%s", template.Id), data, 0).Err()
}
...
func (db *RedisDbDriver) DeleteTemplate(template *model.Template) error {
  ...
  return db.connection.Del(context.Background(), fmt.Sprintf("Templates:%s", template.Id)).Err()
}

func (db *RedisDbDriver) GetTemplate(id string) (*model.Template, error) {
  var template = &model.Template{}

  v, err := db.connection.Get(context.Background(), fmt.Sprintf("Templates:%s", id)).Result()
  if err != nil {
...
func (db *RedisDbDriver) GetTemplates() ([]*model.Template, error) {
  ...
  iter := db.connection.Scan(context.Background(), 0, "Templates:*", 0).Iterator()
  for iter.Next(context.Background()) {
    template, err := db.GetTemplate(iter.Val()[10:])
    if err != nil {
      return nil, err
    }
