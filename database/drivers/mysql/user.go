...
func (db *MySQLDriver) getUsers(where string, args ...interface{}) ([]*model.User, error) {
	...
	rows, err := db.connection.Query(fmt.Sprintf("SELECT user_id, username, email, password, active, updated_at, created_at, last_login_at, ssh_public_key, preferred_shell, roles, groups, timezone, max_spaces, max_disk_space, service_password, github_username FROM users %s ORDER BY username ASC", where), args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&user.Id, &user.Username, &user.Email, &user.Password, &user.Active, &updatedAt, &createdAt, &lastLoginAt, &user.SSHPublicKey, &user.PreferredShell, &roles, &user.Groups, &user.Timezone, &user.MaxSpaces, &user.MaxDiskSpace, &user.ServicePassword, &user.GitHubUsername)
		if err != nil {
			return nil, err
		}
