...
func (db *MySQLDriver) SaveSpace(space *model.Space) error {
	...
	if doUpdate {
		...
		_, err = tx.Exec("INSERT INTO spaces (space_id, user_id, template_id, name, created_at, updated_at, shell, is_deployed, is_pending, is_deleting, volume_data, volume_sizes, nomad_namespace, container_id, template_hash, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			space.Id, space.UserId, space.TemplateId, space.Name, time.Now().UTC(), time.Now().UTC(), space.Shell, space.IsDeployed, space.IsPending, space.IsDeleting, volumeData, volumeSizes, space.NomadNamespace, space.ContainerId, space.TemplateHash, space.Location,
		)
	}
	if err != nil {
		tx.Rollback()
...
func (db *MySQLDriver) getRow(rows *sql.Rows) (*model.Space, string, error) {
	...
	err := rows.Scan(&space.Id, &space.UserId, &space.TemplateId, &space.Name, &createdAt, &updatedAt, &space.Shell, &space.IsDeployed, &space.IsPending, &space.IsDeleting, &volumeData, &space.NomadNamespace, &space.ContainerId, &space.TemplateHash, &volumeSizes, &parentId, &space.Location)
	if err != nil {
		return nil, "", err
	}
...
func (db *MySQLDriver) GetSpace(id string) (*model.Space, error) {
	spaces, err := db.getSpaces("SELECT space_id, user_id, template_id, name, created_at, updated_at, shell, is_deployed, is_pending, is_deleting, volume_data, nomad_namespace, container_id, template_hash, volume_sizes, parent_space_id, location FROM spaces WHERE space_id = ? AND parent_space_id = ''", id)
	if err != nil {
		return nil, err
	}
	if len(spaces) == 0 {
...
func (db *MySQLDriver) GetSpacesForUser(userId string) ([]*model.Space, error) {
	spaces, err := db.getSpaces("SELECT space_id, user_id, template_id, name, created_at, updated_at, shell, is_deployed, is_pending, is_deleting, volume_data, nomad_namespace, container_id, template_hash, volume_sizes, parent_space_id, location FROM spaces WHERE user_id = ? AND parent_space_id = '' ORDER BY name ASC", userId)
	if err != nil {
		return nil, err
	}
...
func (db *MySQLDriver) GetSpaceByName(userId string, spaceName string) (*model.Space, error) {
	...
	rows, err := db.connection.Query("SELECT space_id, user_id, template_id, name, created_at, updated_at, shell, is_deployed, is_pending, is_deleting, volume_data, nomad_namespace, container_id, template_hash, volume_sizes, parent_space_id, location FROM spaces WHERE user_id = ? AND name = ?", userId, spaceName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	if rows.Next() {
		...
		if parentId != "" {
			rows2, err := db.connection.Query("SELECT space_id, user_id, template_id, name, created_at, updated_at, shell, is_deployed, is_pending, is_deleting, volume_data, nomad_namespace, container_id, template_hash, volume_sizes, parent_space_id, location FROM spaces WHERE space_id = ?", parentId)
			if err != nil {
				return nil, err
			}
			defer rows2.Close()
...
func (db *MySQLDriver) GetSpacesByTemplateId(templateId string) ([]*model.Space, error) {
	spaces, err := db.getSpaces("SELECT space_id, user_id, template_id, name, created_at, updated_at, shell, is_deployed, is_pending, is_deleting, volume_data, nomad_namespace, container_id, template_hash, volume_sizes, parent_space_id, location FROM spaces WHERE template_id = ? AND parent_space_id = '' ORDER BY name ASC", templateId)
	if err != nil {
		return nil, err
	}
...
func (db *MySQLDriver) GetSpaces() ([]*model.Space, error) {
	spaces, err := db.getSpaces("SELECT space_id, user_id, template_id, name, created_at, updated_at, shell, is_deployed, is_pending, is_deleting, volume_data, nomad_namespace, container_id, template_hash, volume_sizes, parent_space_id, location FROM spaces WHERE parent_space_id = '' ORDER BY name ASC")
	if err != nil {
		return nil, err
	}
