...
func (db *MySQLDriver) SaveTemplate(template *model.Template) error {
	...
	// Update
	if doUpdate {
		_, err = tx.Exec("UPDATE templates SET name=?, description=?, job=?, volumes=?, hash=?, updated_user_id=?, updated_at=?, groups=?, local_container=?, is_manual=?, with_terminal=?, with_vscode_tunnel=?, with_code_server=? WHERE template_id=?",
			template.Name, template.Description, template.Job, template.Volumes, template.Hash, template.UpdatedUserId, time.Now().UTC(), template.Groups, template.LocalContainer, template.IsManual, template.WithTerminal, template.WithVSCodeTunnel, template.WithCodeServer, template.Id,
		)
	} else {
		_, err = tx.Exec("INSERT INTO templates (template_id, name, description, job, volumes, hash, created_user_id, created_at, updated_user_id, updated_at, groups, local_container, is_manual, with_terminal, with_vscode_tunnel, with_code_server) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
func (db *MySQLDriver) GetTemplate(id string) (*model.Template, error) {
	templates, err := db.getTemplates("SELECT template_id, name, description, job, volumes, hash, created_user_id, created_at, updated_user_id, updated_at, groups, local_container, is_manual, with_terminal, with_vscode_tunnel, with_code_server FROM templates WHERE template_id = ?", id)
	if err != nil {
		return nil, err
	}
	if len(templates) == 0 {
...
func (db *MySQLDriver) GetTemplates() ([]*model.Template, error) {
	return db.getTemplates("SELECT template_id, name, description, job, volumes, hash, created_user_id, created_at, updated_user_id, updated_at, groups, local_container, is_manual, with_terminal, with_vscode_tunnel, with_code_server FROM templates ORDER BY name")
}
