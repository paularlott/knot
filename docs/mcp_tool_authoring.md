...
Knot runs scripts in two environments. MCP tools and event sinks run on the
knot server in the restricted environment: standard library plus data/network
libraries and the `knot.*` API libraries — no system access, no `scriptling.runtime`,
no plugins. Space-side scripts (startup, streaming, health checks) run in the
space with full system access within the container. Serving (JSON-RPC, HTTP,
MCP) and interactive consoles belong to the real Scriptling CLI in the space.
