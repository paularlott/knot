...
var JobsEnableCmd = &cli.Command{
	...
	Description: "Start running a space's scheduled jobs. Requires ~/.knot-jobs.toml to exist in the space.",
	...
	Run: func(ctx context.Context, cmd *cli.Command) error {
		...
		if err != nil {
			...
			return fmt.Errorf("failed to enable job runner: %w", err)
		}
		if !response.Success {
			return fmt.Errorf("failed to enable job runner: %s", response.Error)
		}

		fmt.Printf("Job runner enabled in space '%s'.\n", spaceName)
		return nil
	},
}
