package command_scripts

import (
	"context"
	"fmt"

	"github.com/paularlott/cli"
	"github.com/paularlott/knot/apiclient"
	"github.com/paularlott/knot/command/cmdutil"
	"github.com/paularlott/knot/internal/util"
)

var listCmd = &cli.Command{
	Name:    "list",
	Usage:   "List scripts",
	MaxArgs: cli.NoArgs,
	Run: func(ctx context.Context, cmd *cli.Command) error {
		client, err := cmdutil.GetClient(cmd)
		if err != nil {
			return fmt.Errorf("failed to create API client: %w", err)
		}

		scripts, err := client.GetScripts(ctx)
		if err != nil {
			return fmt.Errorf("error getting scripts: %w", err)
		}

		if scripts.Count == 0 {
			fmt.Println("No scripts found")
			return nil
		}

		global := cmd.GetBool("global")

		var filtered []apiclient.ScriptInfo
		for _, script := range scripts.Scripts {
			if global && script.UserId == "" {
				filtered = append(filtered, script)
			} else if !global && script.UserId != "" {
				filtered = append(filtered, script)
			}
		}

		if len(filtered) == 0 {
			if global {
				fmt.Println("No global scripts found")
			} else {
				fmt.Println("No user scripts found")
			}
			return nil
		}

		table := [][]string{
			{"NAME", "DESCRIPTION", "ACTIVE", "TYPE", "DISCOVERABLE"},
		}
		for _, script := range filtered {
			active := "No"
			if script.Active {
				active = "Yes"
			}
			discoverable := ""
			if script.ScriptType == "tool" {
				if script.Discoverable {
					discoverable = "Yes"
				} else {
					discoverable = "No"
				}
			}
			table = append(table, []string{
				script.Name,
				script.Description,
				active,
				script.ScriptType,
				discoverable,
			})
		}
		util.PrintTable(table)

		return nil
	},
}
