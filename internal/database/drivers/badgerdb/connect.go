...
func (db *BadgerDbDriver) Connect() error {
	log.Debug("db: connecting to BadgerDB")
			...
			if err != nil {
				...
				for _, group := range groups {
					if group.IsDeleted && group.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete group", "group_id", group.Id)
						}
					}
				}
			}
			...
			if err != nil {
				...
				for _, role := range roles {
					if role.IsDeleted && role.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete role", "role_id", role.Id)
						}
					}
				}
			}
			...
			if err != nil {
				...
				for _, space := range spaces {
					if space.IsDeleted && space.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete space", "space_id", space.Id)
						}
					}
				}
			}
			...
			if err != nil {
				...
				for _, template := range templates {
					if template.IsDeleted && template.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete template", "template_id", template.Id)
						}
					}
				}
			}
			...
			if err != nil {
				...
				for _, templateVar := range templateVars {
					if templateVar.IsDeleted && templateVar.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete template var", "template_var_id", templateVar.Id)
						}
					}
				}
			}
			...
			if err != nil {
				...
				for _, user := range users {
					if user.IsDeleted && user.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete user", "user_id", user.Id)
						}
					}
				}
			}
			...
			if err != nil {
				...
				for _, token := range tokens {
					if token.IsDeleted && token.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete token", "token_id", token.Id)
						}
					}
				}
			}
			...
			if err != nil {
				...
				for _, volume := range volumes {
					if volume.IsDeleted && volume.UpdatedAt.Time().Before(before) {
						...
						if err != nil {
							log.WithError(err).Error("db: failed to delete volume", "volume_id", volume.Id)
						}
					}
				}
			}
