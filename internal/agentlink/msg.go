...
func sendMsg(conn net.Conn, commandType CommandType, payload interface{}) error {
	...
	if err != nil {
		log.Error().Err(err).Msg("agent: Failed to marshal message")
		return err
	}
	...
	if err := conn.SetWriteDeadline(deadline); err != nil {
		log.Error().Err(err).Msg("agent: Failed to set write deadline")
		return err
	}
	...
	if err != nil {
		log.Error().Err(err).Msg("agent: Failed to write message length to socket")
		return err
	}

	_, err = conn.Write(data)
	if err != nil {
		log.Error().Err(err).Msg("agent: Failed to write message to socket")
		return err
	}
...
func receiveMsg(conn net.Conn) (*CommandMsg, error) {
	...
	if err := conn.SetReadDeadline(deadline); err != nil {
		log.Error().Err(err).Msg("agent: Failed to set read deadline")
		return nil, err
	}
	...
	if err != nil {
		log.Error().Err(err).Msg("agent: Failed to read response")
		return nil, err
	}
	...
	if err != nil {
		log.Error().Err(err).Msg("agent: Failed to unmarshal message")
		return nil, err
	}
