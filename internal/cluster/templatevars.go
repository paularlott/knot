...
func (c *Cluster) GossipTemplateVar(templateVar *model.TemplateVar) {
	...
	if c.gossipCluster != nil {
		...
		usedNodes := c.gossipInZone(TemplateVarGossipMsg, &templateVars)
		c.gossipCluster.SendExcluding(TemplateVarGossipMsg, &templateVars, usedNodes)
	}
...
func (c *Cluster) gossipTemplateVars() {
	...
	templateVars, err := db.GetTemplateVars()
	if err != nil {
		log.Error().Err(err).Msg("cluster: Failed to get template variables")
		return
	}
