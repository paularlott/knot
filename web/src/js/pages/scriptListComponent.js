  ...
  return {
    ...
    async getScripts(scriptId) {
      const url = scriptId
        ...
        : `/api/scripts?all_zones=${this.showAllZones}`;
            ...
            response.json().then((data) => {
              ...
              scriptList.forEach((script) => {
                ...
                const index = this.scripts.findIndex(
                  (s) => s.script_id === script.script_id,
                );
                if (index >= 0) {
    ...
    editScript(scriptId) {
      ...
      this.scriptFormModal.isUserScript =
        script && script.user_id ? true : false;
      this.scriptFormModal.show = true;
    },
